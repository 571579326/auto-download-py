...
from app.mappers.browser_page_mapper import browser_page_mapper
from app.mappers.browser_window_mapper import browser_window_mapper
from app.runtime.browser.cdp_client import cdp_client
from app.runtime.browser.process_launcher import browser_process_launcher
from app.runtime.browser.selenium_adapter import selenium_adapter
from app.runtime.browser.types import BrowserRuntime, WindowRuntime
...
class BrowserRuntimeManager:
    ...
    def open_window(self, db: Session) -> OpenWindowResponse:
        browser_process_launcher.ensure_profile_dir()
        browser_process_launcher.ensure_browser_executable_path()

        window_id = uuid.uuid4().hex
        db_window = browser_window_mapper.create(db, window_id)

        try:
            browser_runtime, launched_now = self._ensure_browser_runtime_with_state()

            if cdp_client.is_cdp_http_open_page_mode():
                start_url = settings.start_url or 'about:blank'
                page_title = ''
                page_url = start_url
                if not launched_now:
                    try:
                        target_payload = cdp_client.open_url_by_cdp_http(start_url)
                ...
                page_row = browser_page_mapper.create(
                    db=db,
                ...
                browser_window_mapper.update_last_page_info(db, db_window, page_row.title, page_row.url)
                logger.info(
            ...
            browser_window_mapper.invalidate_by_row_id(db, db_window.id)
    ...
    def open_window_pure(
        ...
        try:
            browser_process = browser_process_launcher.launch_browser_pure_process(safe_url, use_new_window)
            ...
            browser_window_mapper.update_last_page_info(db, db_window, '', page_url)
            logger.info(
    ...
    def open_window_selenium(
        ...
        try:
            if not is_port_open('127.0.0.1', settings.debug_port):
                ...
                browser_process = browser_process_launcher.launch_browser_pure_process('', False)
                browser_started_now = True
                browser_process_launcher.wait_for_cdp_ready(browser_process)

            driver = None
            try:
                driver = selenium_adapter.create_selenium_driver()
            ...
            browser_window_mapper.update_last_page_info(db, db_window, title, opened_url)
            logger.info(
    ...
    def list_windows(self, db: Session) -> WindowListResponse:
        rows = browser_window_mapper.list_active(db)
        windows = [
            WindowSummary(
        ...
        with runtime.lock:
            if cdp_client.is_cdp_http_open_page_mode():
                page_row = self._create_cdp_http_page_record(
                    ...
                    sort_no=browser_page_mapper.next_sort_no(db, db_window.id),
                )
                if req.bringToFront:
        ...
        with runtime.lock:
            if cdp_client.is_cdp_http_open_page_mode():
                if request.newTab:
                    ...
                    target_payload = cdp_client.open_url_by_cdp_http(request.url)
    ...
    def open_config_pages(
        ...
        runtime, db_window = self._get_valid_window_runtime(db, window_id)
        from app.mappers.browser_page_config_mapper import browser_page_config_mapper
        config_rows = browser_page_config_mapper.get_valid_configs(db, config_code)
        if not config_rows:
        ...
        with runtime.lock:
            if cdp_client.is_cdp_http_open_page_mode():
                ...
                if request.bringToFront and opened_rows:
                    ...
                    browser_window_mapper.update_last_page_info(db, db_window, opened_rows[-1].title, opened_rows[-1].url)
            ...
            opened_rows = self._open_pages_batch_in_window(
                ...
                items=items,
                start_sort_no=browser_page_mapper.next_sort_no(db, db_window.id),
            )
            if request.bringToFront and opened_rows:
                self._set_active_page(db, db_window, runtime, opened_rows[-1].id)
            else:
    ...
    def list_pages(self, db: Session, window_id: str) -> PageListResponse:
        ...
        with runtime.lock:
            ...
            page_rows = browser_page_mapper.get_valid_pages(db, db_window.id)
            if not page_rows:
    ...
    def close_page(
        ...
        with runtime.lock:
            if cdp_client.is_cdp_http_open_page_mode():
                ...
                remaining_rows = browser_page_mapper.get_valid_pages(db, db_window.id)
                if remaining_rows:
                    ...
                    browser_window_mapper.update_last_page_info(db, db_window, None, None)
    ...
    def reopen_window(self, db: Session, old_window_id: str) -> ReopenWindowResponse:
        old_window = browser_window_mapper.get_by_window_id(db, old_window_id)
        ...
        source_pages = browser_page_mapper.get_source_pages_for_reopen(db, old_window.id)
        if not source_pages:
        ...
        new_window = browser_window_mapper.create(db, new_window_id)
        ...
        try:
            ...
            first_row = browser_page_mapper.create(
                db=db,
            ...
            restored_map: list[tuple] = [(first_source, first_row)]
            restored_count = 1
            ...
            browser_window_mapper.invalidate_by_row_id(db, new_window.id)
    ...
    def _rebuild_browser_runtime_locked(self) -> tuple[BrowserRuntime, bool]:
        ...
        if is_port_open('127.0.0.1', settings.debug_port):
            ...
            try:
                ...
                logger.warning('重新挂接已存在浏览器失败，准备重新拉起浏览器, debugPort=%s, msg=%s', settings.debug_port, exc)
                self._shutdown_browser_runtime()

        browser_process = browser_process_launcher.launch_browser_process()
    ...
    def _attach_browser_runtime(self) -> BrowserRuntime:
        ...
        try:
            ...
            cdp_client.log_cdp_diagnostics(runtime)
    ...
    def _get_valid_window_runtime(self, db: Session, window_id: str) -> tuple[WindowRuntime, object]:
        from app.models.browser_window import AdBrowserWindow
        db_window = browser_window_mapper.get_by_window_id(db, window_id)
    ...
    def _create_cdp_http_page_record(
        ...
        target_payload = cdp_client.open_url_by_cdp_http(safe_url)
        page_title = str(target_payload.get('title') or '')
        page_url = str(target_payload.get('url') or safe_url)
        return browser_page_mapper.create(
            db=db,
            window_row_id=db_window.id,
            title=page_title,
            url=page_url,
    ...
    def _create_new_browser_window_page(self, browser_runtime: BrowserRuntime, url: str) -> Page:
        ...
        if cdp_client.is_native_open_page_mode():
    ...
    def _do_create_new_browser_window_page(self, browser_runtime: BrowserRuntime, url: str) -> Page:
        with browser_runtime.lock:
            ...
            browser_process = browser_process_launcher.launch_browser_new_window_process(url)
    ...
    def _create_managed_page_in_window(
        ...
        next_sort_no = browser_page_mapper.next_sort_no(db, db_window.id)
        page_row = browser_page_mapper.create(
            db=db,
            window_row_id=db_window.id,
            title=self._safe_title(page),
        ...
        if active:
            ...
            try:
                ...
                pass
            browser_window_mapper.update_last_page_info(db, db_window, self._safe_title(opener_page), self._safe_url(opener_page))
        return page_row, page
    ...
    def _open_pages_batch_in_window(
        ...
        for item in items:
            ...
            row = browser_page_mapper.create(
                db=db,
                window_row_id=db_window.id,
                title=self._safe_title(page),
                url=self._safe_url(page),
    ...
    def _resolve_anchor_page_for_window(
        ...
        active_row = browser_page_mapper.get_active_page(db, db_window.id)
        ...
        valid_rows = browser_page_mapper.get_valid_pages(db, db_window.id)
    ...
    def _set_active_page(
        ...
        page_rows = browser_page_mapper.set_active_status(db, db_window.id, active_page_db_id)
        ...
        if active_row is not None:
            ...
            if active_page is not None:
                try:
                    ...
                    pass
            browser_window_mapper.update_last_page_info(db, db_window, active_row.title, active_row.url)
        else:
            browser_window_mapper.update_last_page_info(db, db_window, None, None)

    def _sync_active_status_from_runtime(self, db: Session, db_window, window_runtime: WindowRuntime) -> None:
        valid_rows = browser_page_mapper.get_valid_pages(db, db_window.id)
        if not valid_rows:
    ...
    def _sync_window_snapshots(self, db: Session, db_window, window_runtime: WindowRuntime) -> None:
        valid_rows = browser_page_mapper.get_valid_pages(db, db_window.id)
        ...
        browser_window_mapper.update_last_page_info(db, db_window, active_row.title if active_row else None, active_row.url if active_row else None)

    def _sync_page_snapshot(self, db: Session, page_row, page: Page, auto_commit: bool = True) -> None:
        browser_page_mapper.sync_snapshot(
            db=db,
            page_row=page_row,
            title=self._safe_title(page),
            url=self._safe_url(page),
    ...
    def _invalidate_page_local(
        ...
        browser_page_mapper.invalidate_page(db, page_db_id)
    ...
    def _invalidate_window_local(self, db: Session, window_id: str, close_pages: bool) -> bool:
        ...
        browser_window_mapper.invalidate_window_and_pages(db, db_window)
    ...
    def _open_config_pages_by_selenium_once(
        ...
        browser_process_launcher.ensure_profile_dir()
        browser_process_launcher.ensure_browser_executable_path()
        config_rows = browser_page_config_mapper.get_valid_configs(db, safe_config_code)
        if not config_rows:
